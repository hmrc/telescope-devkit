...
class Ec2Cli(object):
    ...
    def ssh(self, instance_name: str) -> int:
        ...
        with self._console.status("[bold green]Fetching instance IP address...") as status:
    ...
    def tunnel(self, instance_name: str, host: str, port: int) -> int:
        ...
        with self._console.status(
                f"[bold green]Setting up an SSH tunnel to {host}:{port} via {ssh_server_ip_address}... ") as status:
            tunnel = LocalPortForwarding(ssh_server_ip_address, destination_host=host, destination_port=port, local_host=local_host)
            tunnel.start()

        if not tunnel.is_service_reachable():
            self._console.print(f"Unable to reach remote service via {local_host}:{port}")
            tunnel.stop()
            return 1

        if host != local_host:
            self._console.print(f"If you need to access this service via {host} instead of {local_host} make sure to "
                                f"add an entry in the /etc/hosts file")

        self._console.input("\nPress [yellow]<Enter>[/yellow] at any time to stop the SSH tunnel... ")

        with self._console.status(
                f"[bold green]Stopping SSH tunnel to {host}:{port} via {ssh_server_ip_address}...") as status:
            tunnel.stop()
