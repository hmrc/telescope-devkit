...
# This script will iterate through the Telemetry Docker repositories and update its telemetry-docker-resources templates.
# Usage: ./tools/update_docker_templates.sh </path/to/source/root> <Jira Ticket> <Co-Authored by>
#   e.g: ./tools/update_docker_templates.sh /Users/leemyring/source/hmrc "TEL-3074" "Co-authored-by: Stephen Palfreyman <18111914+sjpalf@users.noreply.github.com>"
...
echo "" > "${SRC_ROOT_DIR}/docker_input.txt"

# Create unique-ish Jira ticket suffix
JIRA_SUFFIX=$(date '+%Y-%m-%d')

# Use Silver Searcher to grep through all Cruft controlled Docker templated repos and export list to file
ag --files-with-matches \
   --file-search-regex .cruft.json \
   --hidden \
   "\"template\": \"https:\/\/github\.com\/hmrc\/telemetry\-docker\-resources\",$" | awk -F/ '{print $1}' > "${SRC_ROOT_DIR}/docker_input.txt"
...
do
  (
      ...
      git commit --no-verify --message="${JIRA_TICKET}: update docker templates" --message="${CO_AUTHOR}" && \
      git push --set-upstream origin "${JIRA_TICKET}-${JIRA_SUFFIX}" && \
      gh pr create --fill >> "${SRC_ROOT_DIR}/pr_output.txt" && \
      echo ""
    )
done < "${SRC_ROOT_DIR}/docker_input.txt"
